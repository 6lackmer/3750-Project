...
function createStoredProcedures() {
    ...
    //# region Get Reservations from email
    sql =
        "CREATE PROCEDURE IF NOT EXISTS `get_reservations_from_email`\n" +
        "(\n" +
        "    IN user_email VARCHAR(255)\n" +
        ")\n" +
        "BEGIN\n" +
        "    IF EXISTS (SELECT * FROM account WHERE username = user_email) THEN\n" +
        "        SELECT r.*, st.site_type_name FROM reservation as r\n" +
        "        INNER JOIN account as a\n" +
        "            ON a.account_id = r.account_id\n" +
        "        INNER JOIN site as s\n" +
        "            ON s.site_id = r.site_id\n" +
        "        INNER JOIN site_type as st\n" +
        "            ON st.site_type_id = s.site_type_id\n" +
        "        WHERE username = user_email\n" +
        "        ORDER BY start_date ASC;\n" +
        "    ELSE\n" +
        "        SELECT 'No account found with the provided email.' AS message;\n" +
        "    END IF;\n" +
        "END;";
    ...
    con.query(sql, function (err, results, fields) {
        if (err) {
            ...
            console.log("database.js: procedure get_reservations_from_email created if it didn't exist");
        }
    });
    ...
    sql =
        ...
        "BEGIN\n" +
        "   IF time = '2' THEN \n" +
        "       SELECT reservation.*, site_type_name FROM reservation \n" +
    ...
    //#region Select Invoice by Reservation

    sql =
        "CREATE PROCEDURE IF NOT EXISTS `get_invoice_from_reservation_id`\n" +
        "( \n" +
        "   IN reservation_id INT\n" +
        ") \n" +
        "BEGIN\n" +
        "   SELECT i.*, a.f_name, a.l_name, RIGHT(c.number, 4) AS card_number FROM invoice AS i \n" +
    ...
    con.query(sql, function (err, results, fields) {
        if (err) {
            ...
            console.log("database.js: procedure get_invoice_from_reservation_id created if it didn't exist");
        }
    });
    ...
    //#region Count Today's Reservations
    sql =
    ...
    sql =
        ...
        "BEGIN \n" +
        "    SELECT r.status, r.reservation_id, a.f_name, a.l_name, r.num_nights, s_type.site_type_name, s.site_number \n" +
