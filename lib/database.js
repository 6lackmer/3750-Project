...
function createStoredProcedures() {
    ...
    sql =
        ...
        "BEGIN\n" +
        "   IF time = '1' THEN \n" +
        "       SELECT * FROM reservation WHERE reservation.account_id = account_id AND reservation.status = 'reserved' OR reservation.status = 'booking' ORDER BY reservation.end_date; \n" +
        "   ELSEIF time = '2' THEN \n" +
        "       SELECT * FROM reservation WHERE reservation.account_id = account_id AND reservation.status = 'out' ORDER BY reservation.end_date;\n" +
        "   ELSE \n" +
        "       SELECT * FROM reservation WHERE reservation.account_id = account_id; \n" +
        "   END IF; \n" +
        "END;";
    ...
    sql =
        ...
        ") \n" +
        "BEGIN\n" +
        "   SELECT * FROM reservation WHERE reservation.reservation_id = reservation_id; \n" +
        "END;";
    ...
    //  - Retrieve reservations & innerjoined site_number by 
    //      1) startDate (tentative startDate < saved startDate) 
    //      2) endDate (tentative endDate > saved endDate)
    //      3) max_trailer_length (if my user has a trailer that is 45' long, have it pull all reservations where the max_trailer_length is at least that long. Also if you could Order By length, that would be great) ( I can probably jerry rig this to also produce all reservations for the daily report)
    sql =
        "CREATE PROCEDURE IF NOT EXISTS get_reservations( \n" +
        "    IN start_date DATE, \n" +
        "    IN end_date DATE, \n" +
        "    IN min_trailer_length TINYINT \n" +
        ") \n" +
        "BEGIN \n" +
        "    SELECT  \n" +
        "        reservation.* \n" +
        ...
        "        reservation \n" +
        "    INNER JOIN  \n" +
        "        site ON reservation.site_id = site.site_id \n" +
        "    WHERE  \n" +
        "        reservation.start_date >= start_date \n" +
        "        AND reservation.end_date <= end_date \n" +
        "        AND site.max_trailer_length >= min_trailer_length \n" +
        "    ORDER BY \n" +
        "        site.max_trailer_length; \n" +
        "END;";
