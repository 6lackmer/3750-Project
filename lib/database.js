...
function createStoredProcedures() {
    ...
    sql = "CREATE PROCEDURE IF NOT EXISTS `add_user_account`(\n" +
        ...
        "IF nCount = 0 THEN\n" +
        "INSERT INTO account (username, hashed_password, salt, account_type, f_name, l_name, email, dod_affiliation, dod_status, pcs)\n" +
        "VALUES (username, hashed_password, salt, account_type, f_name, l_name, email, affiliation, status, pcs\n" +
        ");\n" +
        "ELSE\n" +
        "SET result = 1;\n" +
        "END IF;\n" +
    ...
    sql = "CREATE PROCEDURE IF NOT EXISTS `check_credentials`(\n" +
        ...
        "IF nCount = 1 THEN\n" +
        "SET result = (SELECT user_id FROM account WHERE account.username = username AND account.hashed_password=hashed_password\n" +
        ");\n" +
        "ELSE\n" +
        "SET result = 0;\n" +
        "END IF;\n" +
