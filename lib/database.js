...
function createTables() {
    ...
    sql =
        ...
        "   CONSTRAINT fk_account_record_account_id FOREIGN KEY (account_id) REFERENCES account(account_id) \n" +
        ")";
    ...
    con.execute(sql, function (err, results, fields) {
        if (err) {
            ...
            console.log("database.js: table account_record created, and constraints added, if it didn't exist");
        }
    });
...
function createStoredProcedures() {
    ...
    // Authentication
    sql =
        "CREATE PROCEDURE IF NOT EXISTS `add_account_record`\n" +
        "( \n" +
        "   IN record VARCHAR(255), \n" +
        "   IN account_id INT \n" +
        ") \n" +
        "BEGIN\n" +
        "   INSERT INTO account_record (account_id, record) \n" +
        "       VALUES (account_id, record); \n" +
        "END;";
    ...
    con.query(sql, function (err, results, fields) {
        if (err) {
            ...
            console.log("database.js: procedure add_account_record created if it didn't exist");
        }
    });
    ...
    sql =
        ...
        "   SELECT salt INTO salt FROM account WHERE email = email; \n" +
        "END;";
    ...
    sql =
        ...
        "       SELECT account_id INTO user_id FROM account WHERE email = email AND hashed_password = hashed_password LIMIT 1; \n" +
        "   ELSEIF email IS NOT NULL THEN \n" +
        "       SELECT account_id INTO user_id FROM account WHERE email = email LIMIT 1; \n" +
        "   ELSEIF hashed_password IS NOT NULL THEN \n" +
        "       SELECT account_id INTO user_id FROM account WHERE hashed_password = hashed_password LIMIT 1; \n" +
        "   END IF; \n" +
        "END;";
    ...
    sql =
        "CREATE PROCEDURE IF NOT EXISTS `get_account_records_from_account_id`\n" +
        "( \n" +
        "   IN account_id INT\n" +
        ") \n" +
        "BEGIN\n" +
        "   SELECT * FROM account_record WHERE account_id = account_id; \n" +
        "END;";
    ...
    con.query(sql, function (err, results, fields) {
        if (err) {
            ...
            console.log("database.js: procedure get_account_records_from_account_id created if it didn't exist");
        }
    });
    ...
    sql =
        "CREATE PROCEDURE IF NOT EXISTS `update_account_record_from_account_id`\n" +
        "( \n" +
        "   IN account_record_id INT, \n" +
        "   IN new_record VARCHAR(255) \n" +
        ") \n" +
        "BEGIN\n" +
       "    UPDATE account_record SET record = new_record WHERE account_record_id = account_record_id; \n" +
        "END;";
    ...
    con.query(sql, function (err, results, fields) {
        if (err) {
            ...
            console.log("database.js: procedure update_account_record_from_account_id created if it didn't exist");
        }
    });
}
